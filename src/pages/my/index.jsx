...
import { fetchUploadParams, updateUserInfo } from '../../store/actions'
...
const My = () => {
  ...
  // 用户信息
  const [ user, setUser ] = useState( JSON.parse( Taro.getStorageSync( 'profile' ) ) )
  ...
  const handleLogin = () => {
    Taro.login({
      success: res => {
        ...
        Taro.request({
          url: 'http://localhost:4000/api/v1/auth/login/weapp',
          method: 'POST',
  ...
  const handleChooseAvatar = async ( e ) => {
    ...
    dispatch( fetchUploadParams() )
      .then( res => uploadImg( img, e.timeStamp, res ))
  ...
  const uploadImg = ( imgPath, timeStamp, params ) => {
    ...
    Taro.uploadFile({
      ...
      success: () => setImage( `${ host }/avatar/${ timeStamp }.jpeg` )
    })
  }

  // 保存用户信息
  const handleSubmit = () => {
    dispatch( updateUserInfo( { avatar: image, name: nickname, tel } ) )
      .then( res => {
        setUser( res )
  ...
  return (
    <View className='relative h-screen p-[32rpx]'>
      ...
      <View className='flex justify-between items-center mt-[240rpx] z-[99]'>
        ...
        <View
          className='px-[32rpx] py-[16rpx] text-primary-700 text-[28rpx] bg-white rounded-full'
          onClick={ handleLogin }
        >
          登录/注册
        </View>
